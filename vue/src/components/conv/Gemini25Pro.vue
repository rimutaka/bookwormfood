  ...
  <nav class="mx-auto py-4 border-t mt-12 text-xs">
    <ul class="flex flex-wrap font-small">
      ...
      <li class="flex-grow text-center">
        <span class="align-baseline">{{ appVersion }}</span>
      </li>
      <li class="flex-none">
        <div v-if="isAuthenticated">
          <button class="loginBtn" @click="handleLogout" type="button">
            Sign out
          </button>
        </div>
        <div v-else>
...
import { useAuth0 } from '@auth0/auth0-vue' // Assuming this is the Vue equivalent

// Get isAuthenticated, loginWithRedirect, and logout from useAuth0
const { isAuthenticated, loginWithRedirect, logout } = useAuth0()

watchEffect(() => {
  // toggle sign in / sign out button
  // This effect will re-run if isAuthenticated changes.
  // Actual toggling is handled by v-if in the template.
})
  ...
  await logout({
    logoutParams: {
      returnTo:
        ...
        (window.location.port == '80' ? '' : ':' + window.location.port) + // Note: window.location.port is a string
        '/logout',
    },
  })
}
...
const appVersion = computed<string>(() => {
  let version: string = process.env.VUE_APP_BUILD_TS || '' // Use Vue's env var convention
  if (version) {
    version =
      version.substring(5, 6) == '0'
        ? version.substring(6, 16) // do not show leading zero for 08-25
        : version.substring(5, 16)
    // convert it to v.825.1608
    version = 'v.' + version.replace('-', '').replace(':', '').replace(' ', '.')
  }
  return version
})
</script>